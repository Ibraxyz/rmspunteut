...
const LihatBiaya = (props) => {
    ...
    const [d_isLoading, d_data, d_addData, d_getData, d_editData, d_deleteData] = useDataOperations("biaya");
    const deleteData = async () => {
        console.log("deleting data...");
        const currentSelectedIDSTemp = [...currentSelectedIds];
        for (let i = 0; i < currentSelectedIDSTemp.length; i++) {
            await d_deleteData('biaya', currentSelectedIDSTemp[i]);
            console.log(`index at ${i} has been deleted`);
        }
        setMessage("Biaya yang dipilih berhasil dihapus");
    ...
    const editData = async () => {
        ...
        for (let i = 0; i < currentSelectedIDSTemp.length; i++) {
            if(editDataFieldTracker === 'biaya'){
                await d_editData('biaya', editDataDocIdTracker, { biaya : editDataValueTracker });
            }else if(editDataFieldTracker === 'jenis'){
                await d_editData('biaya', editDataDocIdTracker, { jenis : editDataValueTracker });
            }else{
    ...
    useEffect(() => {
        ...
        //this is used for populate table for the first time
        setRowsBasedOnData();
        console.log('data from db server',d_data);
                ...
                isLoading || d_isLoading ?
                    ...
                    <Box sx={{ marginBottom: "100px" }}>
                        <Paper>
                            ...
                            <Divider />
                            <Box sx={{ padding: "10px" }}>
                                <Button sx={{ margin: "5px" }} startIcon={<DeleteIcon />} onClick={deleteData} variant="outlined" disabled={currentSelectedIds.length === 0 ? true : false}>Hapus biaya yang dipilih</Button>

        ...
        if (k_data.length == 0 || b_data.length == 0) {
            ...
            for (var i = 0; i < k_data.length; i++) {
                for (var j = 0; j < b_data.length; j++) {
                    ...
                    obj["jenis"] = b_data[j].jenis == undefined ? "undefined" : b_data[j].jenis;
                    obj["no_kk"] = k_data[i].no_kk == undefined ? "undefined" : k_data[i].no_kk;
                    obj["no_rumah"] = k_data[i].no_rumah == undefined ? "undefined" : k_data[i].no_rumah;
                    obj["no_tagihan"] = md5(k_data[i].no_kk + "" + b_data[j].biaya + "" + tanggalAktif);
                    obj["sisa"] = b_data[j].biaya == undefined ? "undefined" : b_data[j].biaya;
                    obj["sudahLunas"] = false;
                    obj["kelompok_tagihan"] = md5(k_data[i].no_kk + "" + tanggalAktif);
                    obj["status_kelompok_tagihan"] = false;
                    obj["sudah_dibayar"] = 0;
                    obj["tanggal_aktif"] = getTime(tanggalAktif);
                    obj["tanggal_dibayar"] = getTime(tanggalDibayar);
                    obj["tanggal_dibuat"] = Date.now();
                    obj["telp"] = k_data[i].telp == undefined ? "undefined" : k_data[i].telp;
                    try {
                        await t_addData(obj);
